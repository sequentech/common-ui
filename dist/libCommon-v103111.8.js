...
function uiUploader($log) {
    ...
    function ajaxUpload(file, url) {
        ...
        xhr.upload.onprogress = function(event) {
            ...
            i = +Math.floor(Math.log(bytes) / Math.log(1024)), (bytes / Math.pow(1024, i)).toFixed(i ? 1 : 0) + " " + [ "n/a", "bytes", "KiB", "MiB", "GiB", "TB", "PB", "EiB", "ZiB", "YiB" ][isNaN(bytes) ? 0 : i + 1]), 
...
!function($) {
    ...
    function minErr(module) {
        return function() {
            var message, i, obj, code = arguments[0], prefix = "[" + (module ? module + ":" : "") + code + "] ", template = arguments[1], templateArgs = arguments;
            ...
            i = 2; i < arguments.length; i++) message = message + (2 == i ? "?" : "&") + "p" + (i - 2) + "=" + encodeURIComponent("function" == typeof (obj = arguments[i]) ? obj.toString().replace(/ \{[\s\S]*$/, "") : void 0 === obj ? "undefined" : "string" != typeof obj ? JSON.stringify(obj) : obj);
    ...
    function copy(source, destination, stackSource, stackDest) {
        ...
        if (destination) {
            ...
            var result;
            if (isArray(source)) for (var i = destination.length = 0; i < source.length; i++) result = copy(source[i], null, stackSource, stackDest), 
            isObject(source[i]) && (stackSource.push(source[i]), stackDest.push(result)), destination.push(result); else {
                var h = destination.$$hashKey;
                for (var key in isArray(destination) ? destination.length = 0 : forEach(destination, function(value, key) {
                    delete destination[key];
                }), source) result = copy(source[key], null, stackSource, stackDest), isObject(source[key]) && (stackSource.push(source[key]), 
                stackDest.push(result)), destination[key] = result;
    ...
    function shallowCopy(src, dst) {
        ...
        } else if (isObject(src)) for (var key in dst = dst || {}, src) !hasOwnProperty.call(src, key) || "$" === key.charAt(0) && "$" === key.charAt(1) || (dst[key] = src[key]);
        return dst || src;
    }
    ...
    function jqLitePatchJQueryRemove(name, dispatchThis, filterElems, getterIfNoArguments) {
        ...
        function removePatch(param) {
            ...
            if (!getterIfNoArguments || null != param) for (;list.length; ) for (setIndex = 0, 
            setLength = (set = list.shift()).length; setIndex < setLength; setIndex++) for (element = jqLite(set[setIndex]), 
    ...
    function jqLiteExpandoStore(element, key, value) {
        ...
        expandoStore || (element.ng339 = expandoId = ++jqId, expandoStore = jqCache[expandoId] = {}), 
    ...
    forEach("input,select,option,textarea,button,form,details".split(","), function(value) {
        ...
        css: function(element, name, value) {
            var val;
            if (name = camelCase(name), !isDefined(value)) return msie <= 8 && "" === (val = element.currentStyle && element.currentStyle[name]) && (val = "auto"), 
    ...
    function annotate(fn) {
        ...
        }), fn.$inject = $inject) : isArray(fn) ? (assertArgFn(fn[last = fn.length - 1], "fn"), 
        ...
        function createInternalInjector(cache, factory) {
            ...
            return {
                ...
                instantiate: function(Type, locals) {
                    ...
                    isObject(returnedValue = invoke(Type, instance = new Constructor(), locals)) || isFunction(returnedValue) ? returnedValue : instance;
                },
                get: getService,
                annotate: annotate,
                has: function(name) {
    ...
    function $AnchorScrollProvider() {
        ...
        this.disableAutoScrolling = function() {
            ...
            function scroll() {
                ...
                hash ? (elm = document.getElementById(hash)) ? elm.scrollIntoView() : (list = document.getElementsByName(hash), 
                result = null, forEach(list, function(element) {
    ...
    function Browser(window, document, $log, $sniffer) {
        ...
        self.cookies = function(name, value) {
            ...
            if (!name) {
                if (rawDocument.cookie !== lastCookieString) for (cookieArray = (lastCookieString = rawDocument.cookie).split("; "), 
                lastCookies = {}, i = 0; i < cookieArray.length; i++) 0 < (index = (cookie = cookieArray[i]).indexOf("=")) && (name = unescape(cookie.substring(0, index)), 
            ...
            function cacheFactory(cacheId, options) {
                ...
                return caches[cacheId] = {
                    put: function(key, value) {
                        ...
                        if (!isUndefined(value)) return key in data || size++, data[key] = value, capacity < size && this.remove(staleEnd.key), 
                        value;
                    },
                    get: function(key) {
    ...
    function $CompileProvider($provide, $$sanitizeUriProvider) {
        var hasDirectives = {}, COMMENT_DIRECTIVE_REGEXP = /^\s*directive\:\s*([\d\w_\-]+)\s+(.*)$/, CLASS_DIRECTIVE_REGEXP = /(([\d\w_\-]+)(?:\:([^;]+))?;?)/, EVENT_HANDLER_ATTR_REGEXP = /^(on[a-z]+|formaction)$/;
        this.directive = function registerDirective(name, directiveFactory) {
            return assertNotHasOwnProperty(name, "directive"), isString(name) ? (assertArg(directiveFactory, "directiveFactory"), 
            hasDirectives.hasOwnProperty(name) || (hasDirectives[name] = [], $provide.factory(name + "Directive", [ "$injector", "$exceptionHandler", function($injector, $exceptionHandler) {
                var directives = [];
            ...
            Attributes.prototype = {
                ...
                $set: function(key, value, writeAttr, attrName) {
                    ...
                    ("A" === (nodeName = nodeName_(this.$$element).toUpperCase()) && ("href" === key || "xlinkHref" === key) || "IMG" === nodeName && "src" === key) && (this[key] = value = $$sanitizeUri(value, "src" === key)), 
            ...
            function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
                ...
                (nodeLinkFn = (directives = collectDirectives(nodeList[i], [], attrs, 0 === i ? maxPriority : undefined, ignoreDirective)).length ? applyDirectivesToNode(directives, nodeList[i], attrs, transcludeFn, $rootElement, null, [], [], previousCompileContext) : null) && nodeLinkFn.scope && safeAddClass(attrs.$$element, "ng-scope"), 
                ...
                return linkFnFound ? function(scope, nodeList, $rootElement, parentBoundTranscludeFn) {
                    ...
                    for (n = i = 0, ii = linkFns.length; i < ii; n++) node = stableNodeList[n], nodeLinkFn = linkFns[i++], 
            ...
            function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
                ...
                switch (nodeType) {
                  case 1:
                    addDirective(directives, directiveNormalize(nodeName = nodeName_(node).toLowerCase()), "E", maxPriority, ignoreDirective);
                    for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; j < jj; j++) {
                        ...
                        if (attr = nAttrs[j], !msie || 8 <= msie || attr.specified) {
                            ...
                            ngAttrName === directiveNName + "Start" && (attrEndName = (attrStartName = name).substr(0, name.length - 5) + "end", 
                    ...
                    isString(className = node.className) && "" !== className) for (;match = CLASS_DIRECTIVE_REGEXP.exec(className); ) addDirective(directives, nName = directiveNormalize(match[2]), "C", maxPriority, ignoreDirective) && (attrs[nName] = trim(match[3])), 
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                for (var newScopeDirective, directive, directiveName, $template, linkFn, directiveValue, terminalPriority = -Number.MAX_VALUE, controllerDirectives = previousCompileContext.controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasTemplate = !1, hasElementTranscludeDirective = previousCompileContext.hasElementTranscludeDirective, $compileNode = templateAttrs.$$element = jqLite(compileNode), replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, i = 0, ii = directives.length; i < ii; i++) {
                    ...
                    assertNoDuplicate("'" + directiveName + "' controller", (controllerDirectives = controllerDirectives || {})[directiveName], directive, $compileNode), 
                    ...
                    nonTlbTranscludeDirective = directive), childTranscludeFn = "element" == directiveValue ? (hasElementTranscludeDirective = !0, 
                    ...
                    compile($template, transcludeFn, terminalPriority, replaceDirective && replaceDirective.name, {
                        nonTlbTranscludeDirective: nonTlbTranscludeDirective
                    })) : ($template = jqLite(jqLiteClone(compileNode)).contents(), $compileNode.empty(), 
                    compile($template, transcludeFn))), directive.template) if (hasTemplate = !0, assertNoDuplicate("template", templateDirective, directive, $compileNode), 
                ...
                function nodeLinkFn(childLinkFn, scope, linkNode, $rootElement, boundTranscludeFn) {
                    ...
                    for (transcludeFn = boundTranscludeFn && function(scope, cloneAttachFn) {
                        ...
                        "@" == (controller = directive.controller) && (controller = attrs[directive.name]), 
                    ...
                    i = postLinkFns.length - 1; 0 <= i; i--) try {
                        ...
                        compile: function() {
                            return {
                                pre: function(scope, element, attr) {
                                    ...
                                    }(node, name))) && (attr[name] = interpolateFn(scope), ($$observers[name] || ($$observers[name] = [])).$$inter = !0, 
    ...
    function $ControllerProvider() {
        ...
        this.register = function(name, constructor) {
            ...
            return function(expression, locals) {
                ...
                if (isString(expression) && (constructor = (match = expression.match(CNTRL_REG))[1], 
                identifier = match[3], assertArgFn(expression = controllers.hasOwnProperty(constructor) ? controllers[constructor] : getter(locals.$scope, constructor, !0) || getter($window, constructor, !0), constructor, !0)), 
                instance = $injector.instantiate(expression, locals), identifier) {
                ...
                var chain = [ function(config) {
                    ...
                    function(config, reqData, reqHeaders) {
                        var cache, cachedResp, deferred = $q.defer(), promise = deferred.promise, url = function(url, params) {
                            ...
                            }), 0 < parts.length && (url += (-1 == url.indexOf("?") ? "?" : "&") + parts.join("&")), 
                            url;
                        ...
                        cache) if (isDefined(cachedResp = cache.get(url))) {
                ...
                return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
                    ...
                    if ($browser.$$incOutstandingRequestCount(), url = url || $browser.url(), "jsonp" == lowercase(method)) {
                        ...
                        var jsonpDone = function(url, callbackId, done) {
                            ...
                            script.type = "text/javascript", script.src = url, script.async = !0, addEventListenerFn(script, "load", callback = function(event) {
                            ...
                            }), addEventListenerFn(script, "error", callback), msie <= 8 && (script.onreadystatechange = function() {
                                isString(script.readyState) && /loaded|complete/.test(script.readyState) && (script.onreadystatechange = null, 
                                callback({
                                    type: "load"
                                }));
                            });
                            return rawDocument.body.appendChild(script), callback;
                        ...
                        if (xhr.open(method, url, !0), forEach(headers, function(value, key) {
                            ...
                            if (xhr && 4 == xhr.readyState) {
                                ...
                                -1 !== status && (responseHeaders = xhr.getAllResponseHeaders(), response = "response" in xhr ? xhr.response : xhr.responseText), 
                                -1 === status && msie < 10 || (statusText = xhr.statusText), completeRequest(callback, status || xhr.status, response, responseHeaders, statusText);
                            }
                    ...
                    function completeRequest(callback, status, response, headersString, statusText) {
                        ...
                        callback(status = 1223 === status ? 204 : status, response, headersString, statusText = statusText || ""), 
        ...
        this.$get = [ "$rootScope", "$window", "$q", function($rootScope, $window, $q) {
            ...
            function interval(fn, delay, count, invokeApply) {
                ...
                return count = isDefined(count) ? count : 0, promise.then(null, null, fn), promise.$$intervalId = setInterval(function() {
                    deferred.notify(iteration++), 0 < count && count <= iteration && (deferred.resolve(iteration), 
    ...
    function LocationHtml5Url(appBase, basePrefix) {
        ...
        parseAbsoluteUrl(appBase, this, appBase), this.$$parse = function(url) {
            ...
            return (appUrl = beginsWith(appBase, url)) !== undefined ? rewrittenUrl = (appUrl = beginsWith(basePrefix, prevAppUrl = appUrl)) !== undefined ? appBaseNoFile + (beginsWith("/", appUrl) || appUrl) : appBase + prevAppUrl : (appUrl = beginsWith(appBaseNoFile, url)) !== undefined ? rewrittenUrl = appBaseNoFile + appUrl : appBaseNoFile == url + "/" && (rewrittenUrl = appBaseNoFile), 
    ...
    function $LocationProvider() {
        ...
        this.hashPrefix = function(prefix) {
            ...
            LocationMode = html5Mode ? (appBase = (url = initialUrl).substring(0, url.indexOf("/", url.indexOf("//") + 2)) + (baseHref || "/"), 
            $sniffer.history ? LocationHtml5Url : LocationHashbangInHtml5Url) : (appBase = stripHash(initialUrl), 
            LocationHashbangUrl), ($location = new LocationMode(appBase, "#" + hashPrefix)).$$parseLinkUrl(initialUrl, initialUrl);
    ...
    function setter(obj, path, setValue, fullExp, options) {
        ...
        for (var key, element = path.split("."), i = 0; 1 < element.length; i++) {
            ...
            propertyObj || (propertyObj = {}, obj[key] = propertyObj), (obj = propertyObj).then && options.unwrapPromises && (promiseWarning(fullExp), 
        ...
        return ensureSafeObject(obj[key = ensureSafeMemberName(element.shift(), fullExp)], fullExp), 
        obj[key] = setValue;
    }
    Parser.ZERO = extend(function() {
        ...
        primary: function() {
            ...
            if (this.expect("(")) primary = this.filterChain(), this.consume(")"); else if (this.expect("[")) primary = this.arrayDeclaration(); else if (this.expect("{")) primary = this.object(); else {
                ...
                (primary = token.fn) || this.throwError("not a primary expression", token), primary.literal = !!token.literal, 
                primary.constant = !!token.constant;
            }
            for (;next = this.expect("(", "[", "."); ) "(" === next.text ? (primary = this.functionCall(primary, context), 
        ...
        statements: function() {
            for (var statements = []; ;) if (0 < this.tokens.length && !this.peek("}", ")", ";", "]") && statements.push(this.filterChain()), 
        ...
        objectIndex: function(obj) {
            ...
            return this.consume("]"), extend(function(self, locals) {
                ...
                return ensureSafeMemberName(i, parser.text), o ? ((v = ensureSafeObject(o[i], parser.text)) && v.then && parser.options.unwrapPromises && ("$$v" in (p = v) || (p.$$v = undefined, 
                p.then(function(val) {
                    p.$$v = val;
                })), v = v.$$v), v) : undefined;
            }, {
        ...
        functionCall: function(fn, contextGetter) {
            var argsFn = [];
            if (")" !== this.peekToken().text) for (;argsFn.push(this.expression()), this.expect(","); ) ;
            this.consume(")");
            var parser = this;
            return function(scope, locals) {
                ...
                return ensureSafeObject(context, parser.text), function(obj, fullExpression) {
                    if (obj) {
                        if (obj.constructor === obj) throw $parseMinErr("isecfn", "Referencing Function in Angular expressions is disallowed! Expression: {0}", fullExpression);
    ...
    var getterFnCacheDefault = {}, getterFnCacheExpensive = {};
    ...
    function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
        ...
        return options.unwrapPromises ? function(scope, locals) {
            ...
            return null == pathVal ? pathVal : ((pathVal = eso0(pathVal[key0])) && pathVal.then && (promiseWarning(fullExp), 
            "$$v" in pathVal || ((promise = pathVal).$$v = undefined, promise.then(function(val) {
                promise.$$v = eso0(val);
            })), pathVal = eso0(pathVal.$$v)), key1 ? null == pathVal ? undefined : ((pathVal = eso1(pathVal[key1])) && pathVal.then && (promiseWarning(fullExp), 
            "$$v" in pathVal || ((promise = pathVal).$$v = undefined, promise.then(function(val) {
                promise.$$v = eso1(val);
            })), pathVal = eso1(pathVal.$$v)), key2 ? null == pathVal ? undefined : ((pathVal = eso2(pathVal[key2])) && pathVal.then && (promiseWarning(fullExp), 
            "$$v" in pathVal || ((promise = pathVal).$$v = undefined, promise.then(function(val) {
                promise.$$v = eso2(val);
            })), pathVal = eso2(pathVal.$$v)), key3 ? null == pathVal ? undefined : ((pathVal = eso3(pathVal[key3])) && pathVal.then && (promiseWarning(fullExp), 
            "$$v" in pathVal || ((promise = pathVal).$$v = undefined, promise.then(function(val) {
                promise.$$v = eso3(val);
            })), pathVal = eso3(pathVal.$$v)), key4 ? null == pathVal ? undefined : ((pathVal = eso4(pathVal[key4])) && pathVal.then && (promiseWarning(fullExp), 
            "$$v" in pathVal || ((promise = pathVal).$$v = undefined, promise.then(function(val) {
                promise.$$v = eso4(val);
    ...
    function getterFn(path, options, fullExp) {
        ...
        if (options.csp) fn = pathKeysLength < 6 ? cspSafeGetterFn(pathKeys[0], pathKeys[1], pathKeys[2], pathKeys[3], pathKeys[4], fullExp, options) : function(scope, locals) {
            for (var val, i = 0; val = cspSafeGetterFn(pathKeys[i++], pathKeys[i++], pathKeys[i++], pathKeys[i++], pathKeys[i++], fullExp, options)(scope, locals), 
            locals = undefined, scope = val, i < pathKeysLength; ) ;
            return val;
        }; else {
            var code = "var p;\n";
            ...
            evaledFnGetter.toString = valueFn(code), (needsEnsureSafeObject || options.unwrapPromises) && (evaledFnGetter = function(fn, fullExpression) {
                return function(s, l) {
                    return fn(s, l, promiseWarning, ensureSafeObject, fullExpression);
                };
            }(evaledFnGetter, fullExp)), fn = evaledFnGetter;
        }
        ...
        this.digestTtl = function(value) {
            ...
            Scope.prototype = {
                ...
                $new: function(isolate) {
                    var child;
                    return isolate ? ((child = new Scope()).$root = this.$root, child.$$asyncQueue = this.$$asyncQueue, 
                    ...
                    (child.this = child).$parent = this, child.$$prevSibling = this.$$childTail, this.$$childHead ? (this.$$childTail.$$nextSibling = child, 
                ...
                $watchCollection: function(obj, listener) {
                    var newValue, oldValue, veryOldValue, self = this, trackVeryOldValue = 1 < listener.length, changeDetected = 0, objGetter = $parse(obj), internalArray = [], internalObject = {}, initRun = !0, oldLength = 0;
                    return this.$watch(function() {
                        ...
                        if (isObject(newValue = objGetter(self))) if (isArrayLike(newValue)) {
                            ...
                            changeDetected++), newLength = 0, newValue) newValue.hasOwnProperty(key) && (newLength++, 
                            oldValue.hasOwnProperty(key) ? oldValue[key] != oldValue[key] && newValue[key] != newValue[key] || oldValue[key] === newValue[key] || (changeDetected++, 
                            oldValue[key] = newValue[key]) : (oldLength++, oldValue[key] = newValue[key], changeDetected++));
                            if (newLength < oldLength) for (key in changeDetected++, oldValue) oldValue.hasOwnProperty(key) && !newValue.hasOwnProperty(key) && (oldLength--, 
                            delete oldValue[key]);
                    ...
                    do {
                        ...
                        traverseScopesLoop: do {
                            if (watchers = current.$$watchers) for (length = watchers.length; length--; ) try {
                                ...
                                watch.fn(value, last === initWatchVal ? value : last, current), ttl < 5 && (watchLog[logIdx = 4 - ttl] || (watchLog[logIdx] = []), 
        ...
        return isDefined(matchers) && forEach(matchers, function(matcher) {
            adjustedMatchers.push(function(matcher) {
                ...
                if (isString(matcher)) {
                    ...
                    return matcher = (s = matcher, s.replace(/([-()\[\]{}+?*.$\^|,:#<!\\])/g, "\\$1").replace(/\x08/g, "\\x08")).replace("\\*\\*", ".*").replace("\\*", "[^:/.?&;]*"), 
                    new RegExp("^" + matcher + "$");
                }
    ...
    function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
        ...
        if (hasExponent) 0 < fractionSize && -1 < number && number < 1 && (formatedText = number.toFixed(fractionSize)); else {
            var fractionLen = (numStr.split(DECIMAL_SEP)[1] || "").length;
            isUndefined(fractionSize) && (fractionSize = Math.min(Math.max(pattern.minFrac, fractionLen), pattern.maxFrac)), 
            0 === (number = +(Math.round(+(number.toString() + "e" + fractionSize)).toString() + "e" + -fractionSize)) && (isNegative = !1);
    ...
    var DATE_FORMATS = {
        ...
        Z: function(date) {
            var zone = -1 * date.getTimezoneOffset(), paddedZone = 0 <= zone ? "+" : "";
    ...
    function dateFilter($locale) {
        ...
        return function(date, format) {
            ...
            isString(date) && (date = NUMBER_STRING.test(date) ? int(date) : function(string) {
                var match;
    ...
    function limitToFilter() {
        return function(input, limit) {
            return isArray(input) || isString(input) ? (limit = Math.abs(Number(limit)) === 1 / 0 ? Number(limit) : int(limit)) ? 0 < limit ? input.slice(0, limit) : input.slice(limit) : isString(input) ? "" : [] : input;
        };
    }
    function orderByFilter($parse) {
        return function(array, sortPredicate, reverseOrder) {
            return isArrayLike(array) ? (0 === (sortPredicate = isArray(sortPredicate) ? sortPredicate : [ sortPredicate ]).length && (sortPredicate = [ "+" ]), 
            ...
            }), sortPredicate = results, slice.call(array).sort(reverseComparator(function(o1, o2) {
    ...
    function FormController(element, attrs, $scope, $animate) {
        ...
        toggleValidCss(!0), form.$addControl = function(control) {
            ...
            if (isValid) queue && (arrayRemove(queue, control), queue.length || (--invalidCount || (toggleValidCss(isValid), 
            form.$valid = !0, form.$invalid = !1), toggleValidCss(!(errors[validationToken] = !1), validationToken), 
            parentForm.$setValidity(validationToken, !0, form))); else {
    ...
    function testFlags(validity, flags) {
        var i;
        if (flags) for (i = 0; i < flags.length; ++i) if (validity[flags[i]]) return !0;
        return !1;
    }
    function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        var listener = function(ev) {
            if (!composing) {
                ...
                if (msie && "input" === (ev || noevent).type && element[0].placeholder !== placeholder) placeholder = element[0].placeholder; else {
        ...
        if (pattern) {
            ...
            patternValidator = (match = pattern.match(/^\/(.*)\/([gim]*)$/)) ? (pattern = new RegExp(match[1], match[2]), 
            function(value) {
    ...
    var ngIfDirective = [ "$animate", function($animate) {
                    ...
                    scope.$watch($sce.parseAsResourceUrl(srcExp), function(src) {
                        ...
                        src ? ($http.get(src, {
                            ...
                            if (thisChangeId === changeCounter) {
                                ...
                                currentElement = clone, (currentScope = newScope).$emit("$includeContentLoaded"), 
        ...
        return {
            ...
            link: function($scope, $element, $attr, ctrl, $transclude) {
                ...
                if (lhs = match[1], rhs = match[2], (trackByExp = match[3]) ? (trackByExpGetter = $parse(trackByExp), 
                ...
                $scope.$watchCollection(rhs, function(collection) {
                    ...
                    if (isArrayLike(collection)) collectionKeys = collection, trackByIdFn = trackByIdExpFn || trackByIdArrayFn; else {
                        for (key in trackByIdFn = trackByIdExpFn || trackByIdObjFn, collectionKeys = [], 
                        collection) collection.hasOwnProperty(key) && "$" != key.charAt(0) && collectionKeys.push(key);
                        collectionKeys.sort();
                    }
                    for (arrayLength = collectionKeys.length, length = nextBlockOrder.length = collectionKeys.length, 
                    index = 0; index < length; index++) if (assertNotHasOwnProperty(trackById = trackByIdFn(key = collection === collectionKeys ? index : collectionKeys[index], value = collection[key], index), "`track by` id"), 
                    ...
                    for (key in lastBlockMap) lastBlockMap.hasOwnProperty(key) && (elementsToRemove = getBlockElements((block = lastBlockMap[key]).clone), 
                    $animate.leave(elementsToRemove), forEach(elementsToRemove, function(element) {
                        element.$$NG_REMOVED = !0;
                    }), block.scope.$destroy());
                    for (index = 0, length = collectionKeys.length; index < length; index++) {
                        if (value = collection[key = collection === collectionKeys ? index : collectionKeys[index]], 
                        block = nextBlockOrder[index], nextBlockOrder[index - 1] && (previousNode = getBlockEnd(nextBlockOrder[index - 1])), 
                        block.scope) {
                            for (childScope = block.scope, nextNode = previousNode; (nextNode = nextNode.nextSibling) && nextNode.$$NG_REMOVED; ) ;
            ...
            link: function(scope, element, attr, ctrls) {
                if (ctrls[1]) {
                    ...
                    selectCtrl.init(ngModelCtrl, nullOption, unknownOption), multiple && (ngModelCtrl.$isEmpty = function(value) {
                        ...
                        var displayFn = $parse(match[2] || match[1]), valueName = match[4] || match[6], keyName = match[5], groupByFn = $parse(match[3] || ""), valueFn = $parse(match[2] ? match[1] : valueName), valuesFn = $parse(match[7]), trackFn = match[8] ? $parse(match[8]) : null, optionGroupsCache = [ [ {
                        ...
                        selectElement.empty(), selectElement.on("change", function() {
                            scope.$apply(function() {
                                ...
                                if (multiple) {
                                    for (value = [], groupIndex = 0, groupLength = optionGroupsCache.length; groupIndex < groupLength; groupIndex++) for (index = 1, 
                                    length = (optionGroup = optionGroupsCache[groupIndex]).length; index < length; index++) if ((optionElement = optionGroup[index].element)[0].selected) {
                        ...
                        function render() {
                            var optionGroupName, optionGroup, option, existingParent, existingOptions, existingOption, key, groupLength, length, groupIndex, index, selected, lastElement, element, label, optionGroups = {
                                "": []
                            }, optionGroupNames = [ "" ], modelValue = ctrl.$modelValue, values = valuesFn(scope) || [], keys = keyName ? sortedKeys(values) : values, locals = {}, selectedSet = function() {
                                var selectedSet = !1;
                                if (multiple) {
                            ...
                            for (multiple || (nullOption || null === modelValue ? optionGroups[""].unshift({
                                ...
                                (existingParent = existingOptions[0]).label != optionGroupName && existingParent.element.attr("label", existingParent.label = optionGroupName)), 
...
function(window, angular, undefined) {
    ...
    var moment, i, global = this, round = Math.round, YEAR = 0, MONTH = 1, DATE = 2, HOUR = 3, MINUTE = 4, SECOND = 5, MILLISECOND = 6, languages = {}, momentProperties = {
    ...
    function leftZeroFill(number, targetLength, forceSign) {
        for (var output = "" + Math.abs(number), sign = 0 <= number; output.length < targetLength; ) output = "0" + output;
    ...
    function toInt(argumentForCoercion) {
        ...
        return 0 !== coercedNumber && isFinite(coercedNumber) && (value = 0 <= coercedNumber ? Math.floor(coercedNumber) : Math.ceil(coercedNumber)), 
        value;
    }
    ...
    function checkOverflow(m) {
        var overflow;
        m._a && -2 === m._pf.overflow && (overflow = m._a[MONTH] < 0 || 11 < m._a[MONTH] ? MONTH : m._a[DATE] < 1 || m._a[DATE] > daysInMonth(m._a[YEAR], m._a[MONTH]) ? DATE : m._a[HOUR] < 0 || 23 < m._a[HOUR] ? HOUR : m._a[MINUTE] < 0 || 59 < m._a[MINUTE] ? MINUTE : m._a[SECOND] < 0 || 59 < m._a[SECOND] ? SECOND : m._a[MILLISECOND] < 0 || 999 < m._a[MILLISECOND] ? MILLISECOND : -1, 
        m._pf._overflowDayOfYear && (overflow < YEAR || DATE < overflow) && (overflow = DATE), 
    ...
    function getLangDefinition(key) {
        ...
        for (;i < key.length; ) {
            for (j = (split = normalizeLanguage(key[i]).split("-")).length, next = (next = normalizeLanguage(key[i + 1])) ? next.split("-") : null; 0 < j; ) {
    ...
    function formatMoment(m, format) {
        return m.isValid() ? (format = expandFormat(format, m.lang()), formatFunctions[format] || (formatFunctions[format] = function(format) {
        ...
        }(format)), formatFunctions[format](m)) : m.lang().invalidDate();
    }
    ...
    function timezoneMinutesFromString(string) {
        var possibleTzMatches = (string = string || "").match(parseTokenTimezone) || [], parts = ((possibleTzMatches[possibleTzMatches.length - 1] || []) + "").match(parseTimezoneChunker) || [ "-", 0, 0 ], minutes = 60 * parts[1] + toInt(parts[2]);
    ...
    function addTimeToArrayFromToken(token, input, config) {
        ...
        switch (token) {
          ...
          case "MMM":
          case "MMMM":
            null != (a = getLangDefinition(config._l).monthsParse(input)) ? datePartArray[MONTH] = a : config._pf.invalidMonth = input;
            break;
    ...
    function dateFromConfig(config) {
        ...
        if (!config._d) {
            ...
            }(config), config._w && null == config._a[DATE] && null == config._a[MONTH] && (fixYear = function(val) {
                var int_val = parseInt(val, 10);
                return val ? val.length < 3 ? 68 < int_val ? 1900 + int_val : 2e3 + int_val : int_val : null == config._a[YEAR] ? moment().weekYear() : config._a[YEAR];
            }, temp = null != (w = config._w).GG || null != w.W || null != w.E ? dayOfYearFromWeeks(fixYear(w.GG), w.W || 1, w.E, 4, 1) : (lang = getLangDefinition(config._l), 
            ...
            dayOfYearFromWeeks(fixYear(w.gg), week, weekday, lang._week.doy, lang._week.dow)), 
            ...
            config._d = (config._useUTC ? makeUTCDate : function(y, m, d, h, M, s, ms) {
                var date = new Date(y, m, d, h, M, s, ms);
                y < 1970 && date.setFullYear(y);
                return date;
            }).apply(null, input);
        }
    }
    function makeDateFromStringAndFormat(config) {
        ...
        (parsedInput = (string.match(getParseRegexForToken(token, config)) || [])[0]) && (0 < (skipped = string.substr(0, string.indexOf(parsedInput))).length && config._pf.unusedInput.push(skipped), 
        ...
        config._pf.charsLeftOver = stringLength - totalParsedInputLength, 0 < string.length && config._pf.unusedInput.push(string), 
    ...
    function makeDateFromInput(config) {
        ...
        input === undefined ? config._d = new Date() : matched ? config._d = new Date(+matched[1]) : "string" == typeof input ? function(config) {
        ...
        }(config) : isArray(input) ? (config._a = input.slice(0), dateFromConfig(config)) : function(input) {
            return "[object Date]" === Object.prototype.toString.call(input) || input instanceof Date;
        }(input) ? config._d = new Date(+input) : "object" == typeof input ? function(config) {
            var normalizedInput;
        ...
        }(config) : config._d = new Date(input);
    }
    ...
    function weekOfYear(mom, firstDayOfWeek, firstDayOfWeekOfYear) {
        ...
        return end < daysToDayOfWeek && (daysToDayOfWeek -= 7), daysToDayOfWeek < end - 7 && (daysToDayOfWeek += 7), 
    ...
    function dayOfYearFromWeeks(year, week, weekday, firstDayOfWeekOfYear, firstDayOfWeek) {
        var dayOfYear, d = makeUTCDate(year, 0, 1).getUTCDay();
        return {
            year: 0 < (dayOfYear = 7 * (week - 1) + ((weekday = null != weekday ? weekday : firstDayOfWeek) - firstDayOfWeek) + (firstDayOfWeek - d + (firstDayOfWeekOfYear < d ? 7 : 0) - (d < firstDayOfWeek ? 7 : 0)) + 1) ? year : year - 1,
    ...
    function makeMoment(config) {
        ...
        moment.isMoment(input) ? (config = function(m) {
            var i, result = {};
            for (i in m) m.hasOwnProperty(i) && momentProperties.hasOwnProperty(i) && (result[i] = m[i]);
            return result;
        }(input))._d = new Date(+input._d) : format ? isArray(format) ? function(config) {
            var tempConfig, bestMoment, scoreToBeat, i, currentScore;
            if (0 === config._f.length) return config._pf.invalidFormat = !0, config._d = new Date(NaN);
            for (i = 0; i < config._f.length; i++) currentScore = 0, (tempConfig = extend({}, config))._pf = {
                empty: !1,
                unusedTokens: [],
                unusedInput: [],
                overflow: -2,
        ...
        }(config) : makeDateFromStringAndFormat(config) : makeDateFromInput(config), new Moment(config));
    }
    for (formatTokenFunctions.DDDD = padToken(formatTokenFunctions.DDD, 3), extend(Language.prototype, {
        ...
        var c;
        return "boolean" == typeof lang && (strict = lang, lang = undefined), (c = {
            _isAMomentObject: !0
        })._i = input, c._f = format, c._l = lang, c._strict = strict, c._isUTC = !1, c._pf = {
            empty: !1,
            unusedTokens: [],
            unusedInput: [],
            overflow: -2,
        ...
        return "boolean" == typeof lang && (strict = lang, lang = undefined), (c = {
            ...
            _useUTC: !0,
            _isUTC: !0
        })._l = lang, c._i = input, c._f = format, c._strict = strict, c._pf = {
            empty: !1,
            unusedTokens: [],
            unusedInput: [],
            overflow: -2,
        ...
        ret;
    }, moment.version = "2.5.1", moment.defaultFormat = "YYYY-MM-DDTHH:mm:ssZ", moment.updateOffset = function() {}, 
    moment.lang = function(key, values) {
        return key ? (values ? function(key, values) {
            values.abbr = key, languages[key] || (languages[key] = new Language()), languages[key].set(values), 
            languages[key];
        }(normalizeLanguage(key), values) : null === values ? (!function(key) {
            delete languages[key];
        }(key), key = "en") : languages[key] || getLangDefinition(key), (moment.duration.fn._lang = moment.fn._lang = getLangDefinition(key))._abbr) : moment.fn._lang._abbr;
    ...
    for (moment.normalizeUnits = function(units) {
        ...
        add: function(input, val) {
            return addOrSubtractDurationFromMoment(this, "string" == typeof input ? moment.duration(+val, input) : moment.duration(input, val), 1), 
            this;
        },
        ...
        month: function(input) {
            ...
            return null != input ? ("string" == typeof input && "number" != typeof (input = this.lang().monthsParse(input)) || (dayOfMonth = this.date(), 
        ...
        set: function(units, value) {
            return "function" == typeof this[units = normalizeUnits(units)] && this[units](value), 
            this;
        },
        lang: function(key) {
    ...
    for (i in makeGetterAndSetter("year", "FullYear"), moment.fn.days = moment.fn.day, 
    moment.fn.months = moment.fn.month, moment.fn.weeks = moment.fn.week, moment.fn.isoWeeks = moment.fn.isoWeek, 
    moment.fn.toJSON = moment.fn.toISOString, extend(moment.duration.fn = Duration.prototype, {
        ...
        humanize: function(withSuffix) {
            var milliseconds, withoutSuffix, lang, seconds, minutes, hours, days, years, args, difference = +this, output = (milliseconds = difference, 
            withoutSuffix = !withSuffix, lang = this.lang(), seconds = round(Math.abs(milliseconds) / 1e3), 
            minutes = round(seconds / 60), hours = round(minutes / 60), days = round(hours / 24), 
            years = round(days / 365), (args = seconds < 45 && [ "s", seconds ] || 1 === minutes && [ "m" ] || minutes < 45 && [ "mm", minutes ] || 1 === hours && [ "h" ] || hours < 22 && [ "hh", hours ] || 1 === days && [ "d" ] || days <= 25 && [ "dd", days ] || days <= 45 && [ "M" ] || days < 345 && [ "MM", round(days / 30) ] || 1 === years && [ "y" ] || [ "yy", years ])[2] = withoutSuffix, 
            args[3] = 0 < milliseconds, args[4] = lang, function(string, number, withoutSuffix, isFuture, lang) {
    ...
    }), unitMillisecondFactors) unitMillisecondFactors.hasOwnProperty(i) && (makeDurationAsGetter(i, unitMillisecondFactors[i]), 
        ...
        return angular.module("angularMoment", []).constant("angularMomentConfig", {
            ...
            return function(scope, element, attr) {
                ...
                unwatchChanges = scope.$watch(modelName, function(value) {
                    if (null == value || "" === value) return cancelTimer(), void (currentValue && (element.text(""), 
            ...
            return function(value, format, suffix) {
                return null == value ? "" : moment.duration(value, format).humanize(suffix);
            };
...
function(window, angular, undefined) {
    ...
    angular.module("ngAnimate", [ "ng" ]).directive("ngAnimateChildren", function() {
        ...
        var noop = angular.noop, forEach = angular.forEach, selectors = $animateProvider.$$selectors, ELEMENT_NODE = 1, rootAnimateState = {
            running: !0
        };
        ...
        $provide.decorator("$animate", [ "$delegate", "$injector", "$sniffer", "$rootElement", "$$asyncCallback", "$rootScope", "$document", function($delegate, $injector, $sniffer, $rootElement, $$asyncCallback, $rootScope, $document) {
            ...
            $rootElement.data("$$ngAnimateState", rootAnimateState), $rootScope.$$postDigest(function() {
            ...
            function blockElementAnimations(element) {
                var data = element.data("$$ngAnimateState") || {};
                data.running = !0, element.data("$$ngAnimateState", data);
            }
            function animationRunner(element, animationEvent, className) {
                ...
                if (node) {
                    ...
                    if (isAnimatableClassName(classes)) {
                        ...
                        return forEach(function(name) {
                        ...
                        }(animationLookup), function(animationFactory) {
                            ...
                            cancel: function() {
                                forEach(beforeCancel, function(cancelFn) {
                                    (cancelFn || noop)(!0);
                                }), beforeComplete(!0), forEach(afterCancel, function(cancelFn) {
                                    (cancelFn || noop)(!0);
                                }), afterComplete(!0);
                            }
                        };
            ...
            return {
                enter: function(element, parentElement, afterElement, doneCallback) {
                    ...
                    $rootScope.$$postDigest(function() {
                        performAnimation("enter", "ng-enter", element = stripCommentsFromElement(element), parentElement, afterElement, noop, doneCallback);
                    });
                },
                leave: function(element, doneCallback) {
                    cancelChildAnimations(element = angular.element(element)), blockElementAnimations(element), 
                    $rootScope.$$postDigest(function() {
                ...
                move: function(element, parentElement, afterElement, doneCallback) {
                    ...
                    $delegate.move(element, parentElement, afterElement), $rootScope.$$postDigest(function() {
                        performAnimation("move", "ng-move", element = stripCommentsFromElement(element), parentElement, afterElement, noop, doneCallback);
                    });
                },
                addClass: function(element, className, doneCallback) {
                    element = stripCommentsFromElement(element = angular.element(element)), performAnimation("addClass", className, element, null, null, function() {
                        $delegate.addClass(element, className);
                    }, doneCallback);
                },
                removeClass: function(element, className, doneCallback) {
                    element = stripCommentsFromElement(element = angular.element(element)), performAnimation("removeClass", className, element, null, null, function() {
                        $delegate.removeClass(element, className);
                    }, doneCallback);
                },
                setClass: function(element, add, remove, doneCallback) {
                    element = stripCommentsFromElement(element = angular.element(element)), performAnimation("setClass", [ add, remove ], element, null, null, function() {
                    ...
                    switch (arguments.length) {
                      case 2:
                        if (value) cleanup(element); else {
                            var data = element.data("$$ngAnimateState") || {};
                            data.disabled = !0, element.data("$$ngAnimateState", data);
                        }
                        break;
            ...
            function performAnimation(animationEvent, className, element, parentElement, afterElement, domOperation, doneCallback) {
                ...
                var skipAnimations, ngAnimateState = element.data("$$ngAnimateState") || {}, runningAnimations = ngAnimateState.active || {}, totalActiveAnimations = ngAnimateState.totalActive || 0, lastAnimation = ngAnimateState.last;
                ...
                skipAnimations || function(element, parentElement) {
                    ...
                    do {
                        ...
                        var isRoot = isMatchingElement(parentElement, $rootElement), state = isRoot ? rootAnimateState : parentElement.data("$$ngAnimateState") || {};
                ...
                }(element, parentElement)) return fireDOMOperation(), fireBeforeCallbackAsync(), 
                fireAfterCallbackAsync(), void closeAnimation();
                var skipAnimation = !1;
                if (0 < totalActiveAnimations) {
                    ...
                    0 < animationsToCancel.length && forEach(animationsToCancel, function(operation) {
                        operation.cancel();
                    });
                }
                ...
                "leave" == animationEvent && element.one("$destroy", function(e) {
                    var element = angular.element(this), state = element.data("$$ngAnimateState");
                    if (state) {
                ...
                function fireDOMCallback(animationPhase) {
                    ...
                    elementEvents && elementEvents[eventName] && 0 < elementEvents[eventName].length && $$asyncCallback(function() {
                ...
                function closeAnimation() {
                    if (!closeAnimation.hasBeenRun) {
                        ...
                        }), element.data("$$ngAnimateState", data))), fireDoneCallbackAsync();
                    }
                }
                totalActiveAnimations++, runningAnimations[className] = runner, element.data("$$ngAnimateState", {
                    last: runner,
            ...
            function cancelChildAnimations(element) {
                ...
                if (node) {
                    var nodes = angular.isFunction(node.getElementsByClassName) ? node.getElementsByClassName("ng-animate") : node.querySelectorAll(".ng-animate");
                    forEach(nodes, function(element) {
                        var data = (element = angular.element(element)).data("$$ngAnimateState");
            ...
            function cleanup(element, className) {
                ...
                rootAnimateState.structural = !1); else if (className) {
                    var data = element.data("$$ngAnimateState") || {}, removeAnimations = !0 === className;
                    !removeAnimations && data.active && data.active[className] && (data.totalActive--, 
                    delete data.active[className]), !removeAnimations && data.totalActive || (element.removeClass("ng-animate"), 
                    element.removeData("$$ngAnimateState"));
                }
            }
            ...
            TRANSITIONEND_EVENT = void 0 === window.ontransitionend && void 0 !== window.onwebkittransitionend ? (CSS_PREFIX = "-webkit-", 
            TRANSITION_PROP = "WebkitTransition", "webkitTransitionEnd transitionend") : (TRANSITION_PROP = "transition", 
            "transitionend"), ANIMATIONEND_EVENT = void 0 === window.onanimationend && void 0 !== window.onwebkitanimationend ? (CSS_PREFIX = "-webkit-", 
            ANIMATION_PROP = "WebkitAnimation", "webkitAnimationEnd animationend") : (ANIMATION_PROP = "animation", 
            "animationend");
            var cancelAnimationReflow, DURATION_KEY = "Duration", PROPERTY_KEY = "Property", DELAY_KEY = "Delay", ANIMATION_ITERATION_COUNT_KEY = "IterationCount", NG_ANIMATE_PARENT_KEY = "$$ngAnimateKey", NG_ANIMATE_CSS_DATA_KEY = "$$ngAnimateCSS3Data", NG_ANIMATE_BLOCK_CLASS_NAME = "ng-animate-block-transitions", ELAPSED_TIME_MAX_DECIMAL_PLACES = 3, CLOSING_TIME_BUFFER = 1.5, ONE_SECOND = 1e3, lookupCache = {}, parentCounter = 0, animationReflowQueue = [];
            ...
            var closingTimer = null, closingTimestamp = 0, animationElementQueue = [];
            function animationCloseHandler(element, totalTime) {
                ...
                closingTimer = $timeout(function() {
                    forEach(animationElementQueue, function(element) {
                ...
                if (!data) {
                    ...
                    forEach(element, function(element) {
                        if (element.nodeType == ELEMENT_NODE) {
                            ...
                            0 < aDuration && (aDuration *= parseInt(elementStyles[ANIMATION_PROP + ANIMATION_ITERATION_COUNT_KEY], 10) || 1), 
            ...
            function animateSetup(animationEvent, element, className, calculationDecorator) {
                ...
                }(element), eventCacheKey = cacheKey + " " + className, itemIndex = lookupCache[eventCacheKey] ? ++lookupCache[eventCacheKey].total : 0, stagger = {};
                if (0 < itemIndex) {
                ...
                var isCurrentlyAnimating = 0 < formerData.running || "setClass" == animationEvent;
                return 0 < transitionDuration && function(element, className, isAnimating) {
                    !function(className) {
                        return "ng-enter" == className || "ng-move" == className || "ng-leave" == className;
                    }(className) && isAnimating ? element.addClass(NG_ANIMATE_BLOCK_CLASS_NAME) : extractElementNode(element).style[TRANSITION_PROP + PROPERTY_KEY] = "none";
                }(element, className, isCurrentlyAnimating), 0 < animationDuration && 0 < stagger.animationDelay && 0 === stagger.animationDuration && function(element) {
                    extractElementNode(element).style[ANIMATION_PROP] = "none 0s";
                }(element), !0;
            }
            function unblockTransitions(element, className) {
                var prop = TRANSITION_PROP + PROPERTY_KEY, node = extractElementNode(element);
                node.style[prop] && 0 < node.style[prop].length && (node.style[prop] = ""), element.removeClass(NG_ANIMATE_BLOCK_CLASS_NAME);
            }
            function unblockKeyframeAnimations(element) {
                var prop = ANIMATION_PROP, node = extractElementNode(element);
                node.style[prop] && 0 < node.style[prop].length && (node.style[prop] = "");
            }
            function prepareStaggerDelay(delayStyle, staggerDelay, index) {
                var style = "";
                return forEach(delayStyle.split(","), function(val, i) {
                    style += (0 < i ? "," : "") + (index * staggerDelay + parseInt(val, 10)) + "s";
                }), style;
            }
            ...
            function animateAfter(animationEvent, element, className, afterAnimationComplete) {
                if (element.data(NG_ANIMATE_CSS_DATA_KEY)) return function(animationEvent, element, className, activeAnimationComplete) {
                    ...
                    if (-1 != node.getAttribute("class").indexOf(className) && elementData) {
                        ...
                        forEach(className.split(" "), function(klass, i) {
                            activeClassName += (0 < i ? " " : "") + klass + "-active";
                        });
                        ...
                        if (0 < timings.transitionDuration) {
                        ...
                        if (0 < itemIndex) {
                            if (0 < stagger.transitionDelay && 0 === stagger.transitionDuration) {
                                var delayStyle = timings.transitionDelayStyle;
                                style += CSS_PREFIX + "transition-delay: " + prepareStaggerDelay(delayStyle, stagger.transitionDelay, itemIndex) + "; ", 
                                appliedStyles.push(CSS_PREFIX + "transition-delay");
                            }
                            0 < stagger.animationDelay && 0 === stagger.animationDuration && (style += CSS_PREFIX + "animation-delay: " + prepareStaggerDelay(timings.animationDelayStyle, stagger.animationDelay, itemIndex) + "; ", 
                            appliedStyles.push(CSS_PREFIX + "animation-delay"));
                        }
                        if (0 < appliedStyles.length) {
                        ...
                        var totalTime = (itemIndex * (Math.max(stagger.animationDelay, stagger.transitionDelay) || 0) + (maxDelay + maxDuration) * CLOSING_TIME_BUFFER) * ONE_SECOND;
                    ...
                    function onAnimationProgress(event) {
                        ...
                        Math.max(timeStamp - startTime, 0) >= maxDelayTime && maxDuration <= elapsedTime && activeAnimationComplete();
                    }
            ...
            function animate(animationEvent, element, className, animationComplete) {
                ...
                return afterReflow(0, function() {
                    unblockTransitions(element), unblockKeyframeAnimations(element), cancel = animateAfter(0, element, className, animationComplete);
                }), function(cancelled) {
            ...
            return {
                ...
                beforeSetClass: function(element, add, remove, animationCompleted) {
                    ...
                    if (cancellationMethod) return afterReflow(0, function() {
                ...
                setClass: function(element, add, remove, animationCompleted) {
                    return animateAfter(0, element, (remove = suffixClasses(remove, "-remove")) + " " + (add = suffixClasses(add, "-add")), animationCompleted);
                },
                addClass: function(element, className, animationCompleted) {
                    return animateAfter(0, element, suffixClasses(className, "-add"), animationCompleted);
                },
                ...
                removeClass: function(element, className, animationCompleted) {
                    return animateAfter(0, element, suffixClasses(className, "-remove"), animationCompleted);
                }
            };
            function suffixClasses(classes, suffix) {
                var className = "";
                return classes = angular.isArray(classes) ? classes : classes.split(/\s+/), forEach(classes, function(klass, i) {
                    klass && 0 < klass.length && (className += (0 < i ? " " : "") + klass + suffix);
                }), className;
            }
        } ]);
    } ]);
...
angular.module("ui.bootstrap.transition", []).factory("$transition", [ "$q", "$timeout", "$rootScope", function($q, $timeout, $rootScope) {
    ...
    var $modalProvider = {
        ...
        $get: [ "$injector", "$rootScope", "$q", "$http", "$templateCache", "$controller", "$modalStack", function($injector, $rootScope, $q, $http, $templateCache, $controller, $modalStack) {
            ...
            return $modal.open = function(modalOptions) {
                ...
                var resolves, promisesArr, options, templateAndResolvePromise = $q.all([ (options = modalOptions, 
                options.template ? $q.when(options.template) : $http.get(options.templateUrl, {
                    cache: $templateCache
    ...
    return {
        ...
        link: function(originalScope, element, attrs, modelCtrl) {
            ...
            var timeoutPromise, resetMatches = function() {
    ...
    return function(mode, scope, elm, attrs) {
        ...
        params = scope.$eval(attrs["ui" + (string = mode, string.charAt(0).toUpperCase() + string.slice(1))]), 
        ...
        registerFor: function(element) {
            ...
            return css = angular.element.prototype.css, element.prototype.css = function(name, value) {
                ...
                height: function(value) {
                    return this, angular.isDefined(value) ? (angular.isNumber(value) && (value += "px"), 
                    css.call(this, "height", value)) : getWidthHeight(this[0], "height", "base");
                },
...
function(window, angular, undefined) {
    ...
    var _cookie = {
        ...
        toLanguages: function(lng) {
            ...
            var languages = [], whitelist = o.lngWhitelist || !1, addLanguage = function(language) {
...
mod.directive("infiniteScroll", [ "$rootScope", "$window", "$interval", "THROTTLE_MILLISECONDS", function(a, b, c, d) {
    return {
        ...
        link: function(e, f, g) {
            ...
            return x = angular.element(b), w = !(q = !(j = i = u = t = null)), p = function(a) {
                ...
                var b, c, d;
                return d = j === x ? (b = p(j) + s(j[0].document.documentElement), r(f) + p(f)) : (b = p(j), 
                void (c = 0) !== r(j) && (c = r(j)), r(f) - c + p(f)), w && (d = p((f[0].ownerDocument || f[0].document).documentElement)), 
                d - b <= p(j) * t + 1 ? (i = !0, u ? e.$$phase || a.$$phase ? e.infiniteScroll() : e.$apply(e.infiniteScroll) : void 0) : i = !1;
            }, v = function(a, b) {
                var d, e, f;
                return f = null, e = 0, d = function() {
                    ...
                    var g, h;
                    return g = new Date().getTime(), (h = b - (g - e)) <= 0 ? (clearTimeout(f), c.cancel(f), 
            ...
            (h = function(a) {
                return null != j && j.unbind("scroll", o), null != (j = a) ? j.bind("scroll", o) : void 0;
            })(x), k = function(a) {
    ...
    function c(a) {
        ...
        this._nextChunk = null, this.isFirstChunk = !0, this._completeResults = {
            ...
            this._handle = new g(b), (this._handle.streamer = this)._config = b;
        }.call(this, a), this.parseChunk = function(a) {
    ...
    function d(a) {
        var d;
        (a = a || {}).chunkSize || (a.chunkSize = y.RemoteChunkSize), c.call(this, a), this._nextChunk = t ? function() {
            ...
            if (4 == d.readyState) {
                if (d.status < 200 || 400 <= d.status) return void this._chunkError();
                this._finished = !this._config.chunkSize || this._start > function(a) {
    ...
    function e(a) {
        (a = a || {}).chunkSize || (a.chunkSize = y.LocalChunkSize), c.call(this, a);
        var b, d, e = "undefined" != typeof FileReader;
        this.stream = function(a) {
            this._input = a, d = a.slice || a.webkitSlice || a.mozSlice, e ? ((b = new FileReader()).onload = p(this._chunkLoaded, this), 
            b.onerror = p(this._chunkError, this)) : b = new FileReaderSync(), this._nextChunk();
    ...
    function f(a) {
        var d;
        c.call(this, a = a || {}), this.stream = function(a) {
            return d = a, this._nextChunk();
        }, this._nextChunk = function() {
            if (!this._finished) {
    ...
    function g(a) {
        ...
        function e(b, c) {
            return !0 === (a.dynamicTyping[b] || a.dynamicTyping) ? "true" === c || "TRUE" === c || "false" !== c && "FALSE" !== c && function(a) {
        ...
        this.parse = function(c, d, e) {
            if (a.newline || (a.newline = function(a) {
                var b = (a = a.substr(0, 1048576)).split("\r"), c = a.split("\n"), d = 1 < c.length && c[0].length < b[0].length;
                if (1 === b.length || d) return "\n";
                for (var e = 0, f = 0; f < b.length; f++) "\n" === b[f][0] && e++;
                return e >= b.length / 2 ? "\r\n" : "\r";
            }(c)), n = !1, a.delimiter) "function" == typeof a.delimiter && (a.delimiter = a.delimiter(c), 
                ...
                var f = function(b, c) {
                    for (var d, e, f, g = [ ",", "\t", "|", ";", y.RECORD_SEP, y.UNIT_SEP ], i = 0; i < g.length; i++) {
                        ...
                        for (var m = new h({
                            ...
                            l += o, void 0 !== f ? 1 < o && (k += Math.abs(o - f), f = o) : f = o;
                        }
                        0 < m.data.length && (l /= m.data.length), (void 0 === e || k < e) && 1.99 < l && (e = k, 
                        d = j);
                    }
                    return {
                        successful: !!(a.delimiter = d),
                        bestDelimiter: d
                    };
                }(c, a.newline);
    ...
    function h(a) {
        var b = (a = a || {}).delimiter, c = a.newline, d = a.comments, e = a.step, f = a.preview, g = a.fastMode, h = a.quoteChar || '"';
        if (("string" != typeof b || -1 < y.BAD_DELIMITERS.indexOf(b)) && (b = ","), d === b) throw "Comment character same as delimiter";
        !0 === d ? d = "#" : ("string" != typeof d || -1 < y.BAD_DELIMITERS.indexOf(d)) && (d = !1), 
        "\n" != c && "\r" != c && "\r\n" != c && (c = "\n");
        var i = 0, j = !1;
        this.parse = function(a, k, l) {
            ...
            function n(b) {
                return l || (void 0 === b && (b = a.substr(i)), y.push(b), i = r, m(y), v && q()), 
                p();
            }
            function o(b) {
            ...
            var r = a.length, s = b.length, t = c.length, u = d.length, v = "function" == typeof e, w = [], x = [], y = [], z = i = 0;
            if (!a) return p();
            if (g || !1 !== g && -1 === a.indexOf(h)) {
                for (var A = a.split(c), B = 0; B < A.length; B++) {
                    ...
                    if (!d || y.substr(0, u) !== d) {
                        ...
                        if (f && f <= B) return w = w.slice(0, f), p(!0);
                    }
                }
                return p();
            }
            ...
            C = a.indexOf(b, i); else {
                ...
                var F = i;
                for (i++; ;) {
                    if (-1 === (F = a.indexOf(h, F + 1))) return l || x.push({
                        type: "Quotes",
                        code: "MissingQuotes",
                    ...
                    if (F === r - 1) return n(a.substring(i, F).replace(E, h));
                    if (a[F + 1] !== h) {
                        if (a[F + 1] === b) {
    ...
    var r, s = "undefined" != typeof self ? self : "undefined" != typeof window ? window : void 0 !== s ? s : {}, t = !s.document && !!s.postMessage, u = t && /(\?|&)papaworker(=|&|$)/.test(s.location.search), v = !1, w = {}, x = 0, y = {};
    if (y.parse = function(a, b) {
        if ((b = b || {}).dynamicTyping = b.dynamicTyping || !1, b.worker && y.WORKERS_SUPPORTED) {
            var c = function() {
                ...
                return b.onmessage = k, b.id = x++, w[b.id] = b;
            }();
        ...
        function e(a, b) {
            ...
            var d = a instanceof Array && 0 < a.length, e = !(b[0] instanceof Array);
            if (d && i) {
                for (var g = 0; g < a.length; g++) 0 < g && (c += j), c += f(a[g], g);
                0 < b.length && (c += k);
            }
        ...
        function f(a, b) {
            return null == a ? "" : (a = a.toString().replace(m, l + l), "boolean" == typeof h && h || h instanceof Array && h[b] || function(a, b) {
                for (var c = 0; c < b.length; c++) if (-1 < a.indexOf(b[c])) return !0;
                return !1;
            }(a, y.BAD_DELIMITERS) || -1 < a.indexOf(j) || " " === a.charAt(0) || " " === a.charAt(a.length - 1) ? l + a + l : a);
        }
        var h = !1, i = !0, j = ",", k = "\r\n", l = '"';
    ...
    }, y.RECORD_SEP = String.fromCharCode(30), y.UNIT_SEP = String.fromCharCode(31), 
            ...
            function b() {
                if (0 !== f.length) {
                    ...
                    if (q(a.before)) {
                        ...
                        if ("object" == typeof e) {
                            if ("abort" === e.action) return void function(b, c, d, e) {
                                q(a.error) && a.error({
                                    name: b
                                }, c, d, e);
                            }("AbortError", b.file, b.inputElem, e.reason);
    ...
    return u ? s.onmessage = function(a) {
        var b = a.data;
    ...
    } : y.WORKERS_SUPPORTED && (r = function() {
        var a = document.getElementsByTagName("script");
        return a.length ? a[a.length - 1].src : "";
    }(), document.body ? document.addEventListener("DOMContentLoaded", function() {
        v = !0;
    }, !0) : v = !0), (d.prototype = Object.create(c.prototype)).constructor = d, (e.prototype = Object.create(c.prototype)).constructor = e, 
    (f.prototype = Object.create(f.prototype)).constructor = f, y;
});
...
var saveAs = saveAs || function(e) {
    ...
    if (!(void 0 === e || "undefined" != typeof navigator && /MSIE [1-9]\./.test(navigator.userAgent))) {
        var t = e.document, n = function() {
        ...
        }, r = t.createElementNS("http://www.w3.org/1999/xhtml", "a"), o = "download" in r, i = /constructor/i.test(e.HTMLElement) || e.safari, f = /CriOS\/[\d]+/.test(navigator.userAgent), u = function(t) {
            ...
            d || (t = p(t));
            var y, v = this, m = "application/octet-stream" === t.type, h = function() {
                l(v, "writestart progress write writeend".split(" "));
            };
            if (v.readyState = v.INIT, o) return y = n().createObjectURL(t), void setTimeout(function() {
                var e, t;
                r.href = y, r.download = u, e = r, t = new MouseEvent("click"), e.dispatchEvent(t), 
                h(), c(y), v.readyState = v.DONE;
            });
            !function() {
                ...
                y || (y = n().createObjectURL(t)), m ? e.location.href = y : e.open(y, "_blank") || (e.location.href = y);
...
"undefined" != typeof module && module.exports ? module.exports.saveAs = saveAs : "undefined" != typeof define && null !== define && null !== define.amd && define("FileSaver.js", function() {
    ...
    this.ResizeSensor = function(element, callback) {
        ...
        function attachResizeEvent(element, resized) {
            ...
            if ("onresize" in element && document.documentMode < 11) element.attachEvent ? element.attachEvent("onresize", function() {
                ...
                element.appendChild(element.resizeSensor), "absolute" !== function(element, prop) {
    ...
    _elementStyle = document.createElement("div").style, _vendor = function() {
        ...
        return ua = window.navigator.userAgent, !!(isOSXFF = /(?=.+Mac OS X)(?=.+Firefox)/.test(ua)) && ((version = /Firefox\/\d{2}\./.exec(ua)) && (version = version[0].replace(/\D+/g, "")), 
        isOSXFF && 23 < +version);
    }, NanoScroll = function() {
    ...
    function e() {
        var a;
        c("Initialising iFrame"), function() {
            function a(a) {
                return "true" === a;
            }
        ...
        }(), void 0 === N && (N = M + "px"), a = "margin", -1 !== N.indexOf("-") && d("Negative CSS value ignored for " + a), 
        h("margin", N), h("background", L), h("padding", O), function() {
        ...
        c('HTML & body height set to "auto"'), bb && (c("Enable public methods"), window.parentIFrame = {
            ...
            !0 === J ? (a(window, "resize", function() {
                D("resize", "Window resized");
            }), a(window, "click", function() {
                D("click", "Window clicked");
            }), b = window.MutationObserver || window.WebKitMutationObserver, b ? Y < 0 ? q() : (a = document.querySelector("body"), 
            d = {
                attributes: !0,
                attributeOldValue: !1,
                characterData: !0,
            ...
            }), c("Enable MutationObserver"), e.observe(a, d)) : (d("MutationObserver not supported in this browser!"), 
            q())) : c("Auto Resize disabled");
            var a, d, e;
            var b;
...
"undefined" != typeof module && module.exports && (module.exports = sjcl), sjcl.cipher.aes = function(a) {
    ...
    encrypt: function(a, b, c, d, e, f) {
        ...
        n = k(n, k((m = l.clamp(k(m.concat([ 0, 0, 0 ]), g), b)).concat([ 0, 0, 0 ]), g)), 
    ...
    decrypt: function(a, b, c, d, e, f) {
        ...
        for (d = d || [], c = 0; c + 4 < s / 32; c += 4) k = l(k, m = l(n, a.decrypt(l(n, b.slice(c, c + 4))))), 
        r = r.concat(m), n = g(n);
        return p = s - 32 * c, k = l(k, m = l(m = a.encrypt(l(n, [ 0, 0, 0, p ])), h.clamp(b.slice(c), p).concat([ 0, 0, 0 ]))), 
        k = a.encrypt(l(k, l(n, g(n)))), d.length && (k = l(k, f ? d : sjcl.mode.ocb2.pmac(a, d))), 
